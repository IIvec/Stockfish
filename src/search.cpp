...
namespace {
  ...
  int stat_bonus(Depth d) {
    return d > 14 ? 66 : 6 * d * d + 247 * d - 158;
  }
...
void Search::init() {
  ...
  for (int i = 1; i < MAX_MOVES; ++i)
      Reductions[i] = int((18.8 + 2 * std::log(Threads.size())) * std::log(i + 0.25 * std::log(i)));
}
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !PvNode
        ...
        && (ss-1)->statScore < 26068
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 29 * depth - 37 * improving + 166 * ss->ttPv + 107
        && !excludedMove
        ...
        && (ss->ply >= thisThread->nmpMinPly || us != thisThread->nmpColor))
    {
        ...
        Depth R = (1161 + 74 * depth) / 256 + std::min(int(eval - beta) / 200, 3);
    ...
    probCutBeta = beta + 278 - 36 * improving;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              || givesCheck)
          {
              ...
              // SEE based pruning
              if (!pos.see_ge(move, Value(-247) * depth)) // (~25 Elo)
                  continue;
          }
          else
          {
              ...
              if (   lmrDepth < 7
                  && !ss->inCheck
                  && ss->staticEval + 159 + 112 * lmrDepth <= alpha
                  &&  (*contHist[0])[movedPiece][to_sq(move)]
                    + (*contHist[1])[movedPiece][to_sq(move)]
                    + (*contHist[3])[movedPiece][to_sq(move)]
                    + (*contHist[5])[movedPiece][to_sq(move)] / 3 < 27962)
                  continue;
      ...
      if (    depth >= 3
          ...
          && (  !captureOrPromotion
              ...
              || thisThread->ttHitAverage < 403 * TtHitAverageResolution * TtHitAverageWindow / 1024)
          && (!PvNode || ss->ply > 1 || thisThread->id() % 4 != 3))
      {
          Depth r = reduction(improving, depth, moveCount);

          // Decrease reduction if the ttHit running average is large
          if (thisThread->ttHitAverage > 562 * TtHitAverageResolution * TtHitAverageWindow / 1024)
              r--;
          ...
          if (captureOrPromotion)
          {
              ...
              if (   !givesCheck
                  && ss->staticEval + PieceValue[EG][pos.captured_piece()] + 199 * depth <= alpha)
                  r++;
          }
          else
          {
              ...
              if (ss->inCheck)
                  r -= (thisThread->mainHistory[us][from_to(move)]
                     + (*contHist[0])[movedPiece][to_sq(move)] - 3731) / 16384;
              else
