...
void Search::init() {
  ...
  for (int imp = 0; imp <= 1; ++imp)
      for (int d = 1; d < 128; ++d)
          ...
          {
              double r = 0.00919 * (23.4 + log(Threads.size()) / 2) * d * (1.0 - exp(-8.0 / d)) * log(mc);
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              delta1 = (prevScore < 0) ? Value(int(20.0 + 0.1 * abs(prevScore))) : Value(26);
              delta2 = (prevScore > 0) ? Value(int(20.0 + 0.1 * abs(prevScore))) : Value(26);
              alpha = std::max(prevScore - delta1,-VALUE_INFINITE);
              beta  = std::min(prevScore + delta2, VALUE_INFINITE);

              // Adjust contempt based on root move's previousScore (dynamic contempt)
              int dct = ct + (ct ? (111 - ct / 2) * prevScore / (abs(prevScore) + 176) : 0);
